...
2. 확인       Google Books 에 그 제목의 영어판이 실제 있는지       ← Actions 가 한다
...
| Open Library | 한국어판이 속한 작품(work)의 대표 제목 | 새로 추가. 저자가 맞을 때만 받는다 |
| Google Books | 후보가 실제 영어판으로 있는지 확인 | 0건 확인. 키 없는 호출이 공용 한도에 걸린 것으로 보인다 → `GOOGLE_BOOKS_API_KEY` Secret 권장 |
...
Secret `ALADIN_TTB_KEY` 가 있으면 알라딘에서 원제를 찾고, `GOOGLE_BOOKS_API_KEY` 가 있으면 Google Books 확인이 한도에 덜 걸립니다. 둘 다 없어도 돕니다.
...
- 확실 — Google Books 에 같은 제목의 영어판이 있거나, 두 곳 이상에서 같은 제목이 나옴
